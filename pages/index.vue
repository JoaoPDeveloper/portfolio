...
const socialIcon = (icon?: string | null) =>
  icon && icon in iconMap ? iconMap[icon] : ArrowUpRight;
...
const localizedShort = (p: typeof projects[0]) =>
  p.shortDescription[locale.value];
...
<template>
  <main class="min-h-screen bg-[#f6f7f4] text-[#121512] dark:bg-[#090d0c] dark:text-[#f5f7f2]">
    ...
    <header class="fixed inset-x-0 top-0 z-30 border-b border-black/5 bg-[#f6f7f4]/80 backdrop-blur-xl dark:border-white/10 dark:bg-[#090d0c]/80">
      <nav class="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-8">
        ...
        <div class="flex items-center gap-2 text-sm">
          ...
          <button
            type="button"
            class="inline-flex h-10 items-center gap-1.5 rounded-full border border-black/10 bg-white/70 px-3 text-sm font-medium text-[#121512] transition-all duration-500 ease-in-out hover:-translate-y-0.5 dark:border-white/10 dark:bg-white/5 dark:text-white"
      ...
      <div class="mx-auto grid w-full max-w-7xl items-center gap-10 px-5 py-16 sm:px-8 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div class="relative min-h-[420px]">
          ...
          <img
            :src="avatarUrl"
            :alt="profile.name"
            class="absolute inset-x-8 bottom-0 mx-auto h-[92%] max-h-[520px] w-auto object-contain  border-r-8"
          />
          <div class="absolute bottom-6 left-6 right-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/10 p-4 text-white backdrop-blur-xl">
            <span class="text-sm text-white/70">{{ t('hero.available') }}</span>
    ...
    <section class="mx-auto grid max-w-7xl gap-10 px-5 py-20 sm:px-8 lg:grid-cols-[0.75fr_1.25fr]">
      <div>
        <p class="text-sm font-semibold uppercase tracking-[0.22em] text-[#11b5a4]">{{ t('about.label') }}</p>
        <h2 class="mt-3 text-3xl font-semibold tracking-normal sm:text-4xl">{{ t('about.title') }}</h2>
      </div>
      <div class="space-y-8">
        ...
        <div class="grid gap-4 md:grid-cols-3">
          <div
            v-for="point in profile.aboutPoints"
            :key="point"
    ...
    <section id="stack" class="border-y border-black/10 bg-white/60 py-16 dark:border-white/10 dark:bg-white/[0.03]">
      <div class="mx-auto max-w-7xl px-5 sm:px-8">
        <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            <p class="text-sm font-semibold uppercase tracking-[0.22em] text-[#11b5a4]">{{ t('stack.label') }}</p>
            <h2 class="mt-3 text-3xl font-semibold tracking-normal">{{ t('stack.title') }}</h2>
          </div>
        ...
        <div class="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          ...
          >
            <img v-if="technology.iconUrl" :src="technology.iconUrl" :alt="technology.name" class="size-9 object-contain" />
    ...
    <section id="projetos" class="mx-auto max-w-7xl px-5 py-20 sm:px-8">
      <div class="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          <p class="text-sm font-semibold uppercase tracking-[0.22em] text-[#11b5a4]">{{ t('projects.label') }}</p>
          <h2 class="mt-3 text-3xl font-semibold tracking-normal sm:text-5xl">{{ t('projects.title') }}</h2>
        </div>
        <p class="max-w-xl text-sm leading-6 text-[#66736e] dark:text-[#9ca9a3]">
      ...
      <div class="mt-12 grid gap-5 lg:grid-cols-3">
        ...
        >
          <div class="aspect-[16/10] overflow-hidden bg-[#dce4df] dark:bg-[#16211e]">
            <img v-if="project.imageUrl" :src="project.imageUrl" :alt="project.title" class="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
          </div>
          <div class="space-y-4 p-5">
            ...
            <p class="text-sm leading-6 text-[#5f6b66] dark:text-[#a8b4ae]">{{ localizedShort(project) }}</p>
    ...
    <footer id="contato" class="border-t border-black/10 bg-[#121512] text-white dark:border-white/10">
      <div class="mx-auto flex max-w-7xl flex-col justify-between gap-8 px-5 py-12 sm:px-8 md:flex-row md:items-center">
        <div>
          <p class="text-sm text-white/55">{{ t('footer.subtitle') }}</p>
          <h2 class="mt-2 text-3xl font-semibold tracking-normal">{{ t('footer.title') }}</h2>
        </div>
